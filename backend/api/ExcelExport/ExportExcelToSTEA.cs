...
public static class ExportToSTEA
{
    public static List<BusinessCase> export(STEAProjectDto project)
    {
        ...
        foreach (var c in project.STEACases)
        {
            ...
            rowCount++;
            businessCase.Exploration = CreateExcelCostRow("Exploration Cost [Expected Real MNOK'21]", project.StartYear,
                c.Exploration, rowCount, 1);
            rowCount++;
            businessCase.Capex =
                CreateExcelCostRow("Capex [Expected Real MNOK'21]", project.StartYear, c.Capex, rowCount, 1);
            rowCount++;
            businessCase.Drilling = CreateExcelCostRow("Drilling", project.StartYear, c.Capex.Drilling, rowCount, 1);
            rowCount++;
            businessCase.OffshoreFacilites = CreateExcelCostRow("Offshore Facilities", project.StartYear,
                c.Capex.OffshoreFacilities, rowCount, 1);
            rowCount++;
            businessCase.CessationOffshoreFacilites = CreateExcelCostRow("Cessation - Offshore Facilities",
                project.StartYear, c.Capex.CessationOffshoreFacilities, rowCount, 1);
            rowCount++;
            businessCase.ProductionAndSalesVolumes =
                new ExcelTableCell(columnNumber(1) + rowCount, "Production And Sales Volumes");
            rowCount++;
            businessCase.TotalAndAnnualOil = CreateExcelVolumeRow("Total And annual Oil/Condensate production [MSm3]",
                project.StartYear,
            ...
            {
                allRows.Add(c.ProductionAndSalesVolumes.TotalAndAnnualOil.Values.Count() +
                            c.ProductionAndSalesVolumes.TotalAndAnnualOil.StartYear);
            }

            if (c.ProductionAndSalesVolumes.TotalAndAnnualSalesGas.Values != null)
            {
                allRows.Add(c.ProductionAndSalesVolumes.TotalAndAnnualSalesGas.Values.Count() +
                            c.ProductionAndSalesVolumes.TotalAndAnnualSalesGas.StartYear);
            }

            if (c.ProductionAndSalesVolumes.Co2Emissions.Values != null)
            {
                allRows.Add(c.ProductionAndSalesVolumes.Co2Emissions.Values.Count() +
                            c.ProductionAndSalesVolumes.Co2Emissions.StartYear);
            }

            var maxYear = allRows.Count() > 0 ? allRows.Max() - project.StartYear : 0;

            businessCase.Header =
                CreateTableHeader(project.StartYear, project.StartYear + maxYear - 1, c.Name, headerRowCount);
    ...
    private static List<ExcelTableCell> CreateExcelCostRow(string title, int ProjectStartYear, TimeSeriesCostDto e,
        int rowCount, double factor)
    {
    ...
    private static void ValueToCells(List<ExcelTableCell> tableCells, int columnCount, int rowCount,
        TimeSeriesDto<double> e, int ProjectStartYear, double factor)
    {
        if (e.Values == null)
        {
            return;
    ...
    private static List<ExcelTableCell> CreateExcelVolumeRow(string title, int ProjectStartYear, TimeSeriesVolumeDto e,
        int rowCount, double factor)
    {
        var tableCells = new List<ExcelTableCell>();
        var columnCount = 1;
    ...
    private static List<ExcelTableCell> CreateExcelMassRow(string title, int ProjectStartYear, TimeSeriesMassDto e,
        int rowCount, double factor)
    {
        var tableCells = new List<ExcelTableCell>();
        var columnCount = 1;
    ...
    {
        var rv = "";
        var newCellNumber = cellNumber;
        var rest = cellNumber % 26;
