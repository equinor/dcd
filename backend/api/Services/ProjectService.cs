    ...
    public Project GetProject(Guid projectId)
    {
        if (_context.Projects != null)
        {
            ...
            var project = _context.Projects!
                ...
                .Include(p =>p.WellProjects!.Where(asset => asset.ProjectId == projectId)).ThenInclude(wellProject => wellProject.OilProducerCostProfile)
                .Include(p =>p.WellProjects!.Where(asset => asset.ProjectId == projectId)).ThenInclude(wellProject => wellProject.GasProducerCostProfile)
                .Include(p =>p.WellProjects!.Where(asset => asset.ProjectId == projectId)).ThenInclude(wellProject => wellProject.WaterInjectorCostProfile)
                .Include(p =>p.WellProjects!.Where(asset => asset.ProjectId == projectId)).ThenInclude(wellProject => wellProject.GasInjectorCostProfile)
                .Include(p =>p.WellProjects!.Where(asset => asset.ProjectId == projectId)).ThenInclude(wellProject => wellProject.WellProjectWells)!.ThenInclude(well => well.DrillingSchedule)
                .Include(p =>p.Explorations!.Where(asset => asset.ProjectId == projectId)).ThenInclude(exploration => exploration.ExplorationWellCostProfile)
                .Include(p =>p.Explorations!.Where(asset => asset.ProjectId == projectId)).ThenInclude(exploration => exploration.AppraisalWellCostProfile)
                .Include(p =>p.Explorations!.Where(asset => asset.ProjectId == projectId)).ThenInclude(exploration => exploration.SidetrackCostProfile)
                .Include(p =>p.Explorations!.Where(asset => asset.ProjectId == projectId)).ThenInclude(exploration => exploration.GAndGAdminCost)
                .Include(p =>p.Explorations!.Where(asset => asset.ProjectId == projectId)).ThenInclude(exploration => exploration.SeismicAcquisitionAndProcessing)
                .Include(p =>p.Explorations!.Where(asset => asset.ProjectId == projectId)).ThenInclude(exploration => exploration.CountryOfficeCost)
                .Include(p =>p.Explorations!.Where(asset => asset.ProjectId == projectId)).ThenInclude(exploration => exploration.ExplorationWells)!.ThenInclude(explorationWell => explorationWell.DrillingSchedule)
                .Include(p =>p.Explorations!.Where(asset => asset.ProjectId == projectId)).ThenInclude(exploration => exploration.ExplorationWells)!.ThenInclude(explorationWell => explorationWell.Well)
                .Include(p =>p.DrainageStrategies!.Where(asset => asset.ProjectId == projectId))
                .Include(p =>p.Substructures!.Where(asset => asset.ProjectId == projectId)).ThenInclude(substructure => substructure.CostProfile)
                .Include(p =>p.Substructures!.Where(asset => asset.ProjectId == projectId)).ThenInclude(substructure => substructure.CessationCostProfile)
                .Include(p =>p.Surfs!.Where(asset => asset.ProjectId == projectId)).ThenInclude(surf => surf.CostProfile)
                .Include(p =>p.Surfs!.Where(asset => asset.ProjectId == projectId)).ThenInclude(surf => surf.CessationCostProfile)
                .Include(p =>p.Topsides!.Where(asset => asset.ProjectId == projectId)).ThenInclude(topside => topside.CostProfile)
                .Include(p =>p.Topsides!.Where(asset => asset.ProjectId == projectId)).ThenInclude(topside => topside.CessationCostProfile)
                .Include(p =>p.Transports!.Where(asset => asset.ProjectId == projectId)).ThenInclude(transport => transport.CostProfile)
