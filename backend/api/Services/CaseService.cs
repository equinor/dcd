...
public class CaseService
{
    ...
    public ProjectDto DuplicateCase(Guid caseId)
    {
        ...
        var duplicateCaseNames = new List<Case>();
        foreach (var c in project.Cases!)
        {
            var copyNumber = c.Name.Substring(c.Name.Length - 1, 1);
    ...
    private static bool IsWellProjectWell(WellCategory wellCategory)
    {
        return new[]
        {
    ...
    public TimeSeries<double> CalculateWellInterventionCostProfile(Guid caseId)
    {
        ...
        var drainageStrategyService =
            (DrainageStrategyService?)_serviceProvider.GetService(typeof(DrainageStrategyService));
