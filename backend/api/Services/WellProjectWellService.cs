...
public class WellProjectWellService
{
    ...
    public WellProjectWellService(DcdDbContext context, ProjectService projectService,
        WellProjectService wellProjectService, ILoggerFactory loggerFactory)
    {
        _context = context;
    ...
    public ProjectDto UpdateWellProjectWell(WellProjectWellDto updatedWellProjectWellDto)
    {
        ...
        var wellProject = _context.WellProjects!.Include(wp => wp.CostProfile).Include(wp => wp.WellProjectWells)
        ...
        _context.SaveChanges();
        var projectId = _context.WellProjects!.FirstOrDefault(c => c.Id == updatedWellProjectWellDto.WellProjectId)
            ?.ProjectId;
        if (projectId != null)
        {
            return _projectService.GetProjectDto((Guid)projectId);
