...
public static class EmissionCalculationHelper
{
    ...
    public static TimeSeries CalculateFlaring(Case caseItem)
    {
        ...
        var gasRate = caseItem.GetProfileOrNull(ProfileTypes.ProductionProfileGas)?.Values.Select(v => v / OilToGasEnergyRatio).ToArray() ?? [];
        ...
        var additionalGasRate = caseItem.GetProfileOrNull(ProfileTypes.AdditionalProductionProfileGas)?.Values.Select(v => v / OilToGasEnergyRatio).ToArray() ?? [];
