...
public static class CaseDtoAdapter
{
    ...
    private static void CalculateCessationCost(Case caseItem, CaseDto caseDto, ProjectDto projectDto)
    {
        ...
        var productionProfileOil = projectDto.DrainageStrategies
            ?.FirstOrDefault(ds => ds.Id == caseItem.DrainageStrategyLink)?.ProductionProfileOil;
        if (productionProfileOil != null)
        {
            var lastYear = productionProfileOil.StartYear + productionProfileOil.Values.Length - 1;
            var linkedWellsDrillingSchedule = projectDto.WellProjects
                ?.FirstOrDefault(wp => wp.Id == caseItem.WellProjectLink)?.WellProjectWells?.Where(wpw => wpw.Count > 0)
                .Select(wpw => wpw.DrillingSchedule);
            var pluggingAndAbandonment = projectDto.WellProjects
                ?.FirstOrDefault(wp => wp.Id == caseItem.WellProjectLink)?.PluggingAndAbandonment;
            if (linkedWellsDrillingSchedule != null && pluggingAndAbandonment != null)
            {
                var linkedWells = linkedWellsDrillingSchedule.Where(lwd => lwd != null && lwd.Values != null)
                    .SelectMany(lwd => lwd.Values);
            ...
            if (surfCessationCost != null)
            {
                ...
                var cessationOffshoreFacilitiesValues = new[]
                    { (double)surfCessationCost / 2, (double)surfCessationCost / 2 };
