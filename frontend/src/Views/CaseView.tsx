    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                setCase(caseResult)
                setDrainageStrategy(projectResult?.drainageStrategies.find((ds) => ds.id === caseResult?.drainageStrategyLink))
                setExploration(projectResult?.explorations.find((e) => e.id === caseResult?.explorationLink))
                setWellProject(projectResult?.wellProjects.find((wp) => wp.id === caseResult?.wellProjectLink))
                setSurf(projectResult?.surfs.find((s) => s.id === caseResult?.surfLink))
                setTopside(projectResult?.topsides.find((t) => t.id === caseResult?.topsideLink))
                setSubstructure(projectResult?.substructures.find((s) => s.id === caseResult?.substructureLink))
                setTransport(projectResult?.transports.find((t) => t.id === caseResult?.transportLink))
            } catch (error) {
